...
| `ParameterPath` | `C:\ApprovedExchange\parameters.json`, resolved copy of [sample parameters](../config/parameters.exchange-only.sample.json). Domain, tenant, operations address and group must match this journey. Licensing owner supplies `entitlement` including current recipient domains, service plans, reference, owner, expiry and verified status. This full manifest requires the applicable EXCHANGE, ATP and THREAT_INTELLIGENCE plans; EOP-only licensing must stop, not mark MDO controls Pass. |
